<?php
/**
 * Custom TutorLMS Lesson Sidebar Template
 * Displays course topics and lessons - content only, no layout
 * 
                ...
                while ($lessons->have_posts()) {
                            ...
                            <a href="<?php echo $show_permalink ? esc_url(get_permalink($quiz->ID)) : '#'; ?>" data-quiz-id="<?php echo esc_attr($quiz->ID); ?>">
                                ...
                                <div class="tutor-d-flex tutor-ml-auto tutor-flex-shrink-0">
                                    ...
                                    if ($time_limit) {
                                        ...
                                        echo '<span class="tutor-course-topic-item-duration tutor-fs-7 tutor-fw-medium tutor-color-muted tutor-mr-8">' . esc_html($formatted_time) . '</span>';
                                    }
                                    ?>
                    ...
                    <?php } elseif ('tutor_zoom_meeting' === $post->post_type) { ?>
                        <div class="tutor-course-topic-item tutor-course-topic-item-zoom<?php echo esc_attr((get_the_ID() == $current_post->ID) ? ' is-active' : ''); ?>">
                            <a href="<?php echo $show_permalink ? esc_url(get_permalink($post->ID)) : '#'; ?>">
                                <div class="tutor-d-flex tutor-mr-32">
                                    <span class="tutor-course-topic-item-icon tutor-icon-brand-zoom-o tutor-mr-8 tutor-mt-2"></span>
                                    <span class="tutor-course-topic-item-title tutor-fs-7 tutor-fw-medium">
                                        <?php echo esc_html($post->post_title); ?>
                                    </span>
                                </div>
                    ...
                    <?php } elseif ('tutor-google-meet' === $post->post_type) { ?>
                        <div class="tutor-course-topic-item tutor-course-topic-item-zoom<?php echo esc_attr(get_the_ID() == $current_post->ID ? ' is-active' : ''); ?>">
                            <a href="<?php echo $show_permalink ? esc_url(get_permalink($post->ID)) : '#'; ?>">
                                <div class="tutor-d-flex tutor-mr-32">
                                    <span class="tutor-course-topic-item-icon tutor-icon-brand-google-meet tutor-mr-8 tutor-mt-2"></span>
                                    <span class="tutor-course-topic-item-title tutor-fs-7 tutor-fw-medium">
                                        <?php echo esc_html($post->post_title); ?>
                                    </span>
                                </div>
                        ...
                        <div class="tutor-course-topic-item tutor-course-topic-item-lesson<?php echo esc_attr(get_the_ID() == $current_post->ID ? ' is-active' : ''); ?>">
                            <a href="<?php echo $show_permalink ? esc_url(get_the_permalink()) : '#'; ?>" data-lesson-id="<?php the_ID(); ?>">
                                <div class="tutor-d-flex tutor-mr-32">
                                    ...
                                    echo '<span class="tutor-course-topic-item-icon tutor-icon-' . esc_attr($tutor_lesson_type_icon) . ' tutor-mr-8 tutor-mt-2"></span>';
                                    ?>
                                    <span class="tutor-course-topic-item-title tutor-fs-7 tutor-fw-medium">
                                        <?php the_title(); ?>
                                    </span>
                                ...
                                <div class="tutor-d-flex tutor-ml-auto tutor-flex-shrink-0">
                                    <?php
                                    if ($play_time) {
                                        echo '<span class="tutor-course-topic-item-duration tutor-fs-7 tutor-fw-medium tutor-color-muted tutor-mr-8">' . esc_html(tutor_utils()->get_optimized_duration($play_time)) . '</span>';
                                    }
